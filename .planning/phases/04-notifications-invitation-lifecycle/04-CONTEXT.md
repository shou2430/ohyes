...
- **D-19:** **The job must be multi-instance safe from day one.** Wrap each run in a PostgreSQL advisory lock (`pg_try_advisory_lock` with a fixed key); an instance that cannot acquire the lock skips that tick. The user intends to scale the backend to **3 replicas**, and this makes the scheduler correct at any replica count for roughly five lines of code and zero new infrastructure. Do not ship a scheduler that assumes a single instance. — **Reversibility:** reversible — the lock is local to the job function.
...
- CLAUDE.md records that a Railway volume can only be mounted by one active deployment. Photo upload (`invitations.py:130`), serving (`routers/photos.py`), deletion, and the Phase 4 cleanup job all depend on `/data/photos`. **This, not the scheduler, is what blocks scaling the backend to 3 replicas** — the other two replicas could neither store nor serve photos. Live verification against Railway's docs was not possible in this session (no network access); confirm before any scale-up. Resolving it requires migrating photos to object storage, which is deferred (see Deferred Ideas). D-19 ensures the scheduler itself is already ready when that day comes.
...
- **Migrate photo storage from the Railway volume to object storage (S3 / Cloudflare R2 / equivalent).** This is the actual prerequisite for scaling the backend beyond one replica. It overturns INFR-02 and CLAUDE.md's locked "Railway volume for photos" decision, and touches `backend/app/utils/photo.py`, `backend/app/routers/photos.py`, and the create/delete/respond/cleanup paths, plus a migration plan for photos already live in production. Its own phase — do not absorb into Phase 4.
- **Backend horizontal scale-up to 3 replicas** — blocked on the above. D-19 removes the scheduler as a blocker; nothing else in Phase 4 needs to change when this happens.
